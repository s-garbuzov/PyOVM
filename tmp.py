...
"""
Seems like there are at least two ways to address the issue:



Spent few hours playing with ILOM CLI, was looking for an option
that might be used to disable pagination for the output of show
commands (as suggested by Richard).
Could not find it.

There is the 'pause_count' option in the '/HOST/console' target that allows
controlling pagination of the output specifically for the 'show /HOST/console/history'
command.

No generic option seems to be available for controlling pagination of the output
for other show commands.

The questions I have:
    Am I just missed "a special command line option" in ILOM CLI?
    
    If answer to the previous question is "NO, then
    Do we expect that ILOM CLI team will provide a such option at some time
    (and we are going to wait when it become available)?

    Or we are going to leverage existing Python code to handle output pagination?
...
-> help /HOST/console
 ...
 /HOST/console : Redirect Host Serial Console to SP
    Targets:
	history : View Host Console Output
	...
	logging : enables/disables console history logging
	logging : Possible values = enabled, disabled
	logging : User role required for set = a
	...
	timestamp : whether console history will be timestamped
	timestamp : Possible values = yes,no
	timestamp : User role required for set = c
...
There are CLI options for control pagination of CLI history 
"""
